...
class Robot:
    ...
    def MoveRobot(self, x, y):
        self.positionX = x  #Culm
        self.positionY = y  #Row
        print("Robot moved to position ["+ str(self.positionX) + "][" + str(self.positionY) + "]")

    #def CleanRobot(path):
     #   for position in path:
      #      G = position[0:2]
       # print("Goal position has been cleaned!" + str(G))
...
def main():
    ...
    # Find the closest path from start(@) to end($)
        ...
        for x, y in path:
            robot.MoveRobot(x,y)
        #robot.CleanRobot(path) 
            
        #for x in path:
         #   print("Robot moved to position"+str(x))
        
        last_element = path[-1]
        print("\nGoal position has been cleaned!"+ str(last_element))
